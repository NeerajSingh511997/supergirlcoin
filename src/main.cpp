...
#if defined(NDEBUG)
# error "SuperGirlCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8e616bd53c8be64599f3ae82758ac53d84c209ff34095e85118ddf9b8ab9539f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SuperGirlCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SuperGirlCoin Signed Message:\n";
...
{
    // SuperGirlCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // SuperGirlCoin: 10k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // SuperGirlCoin: 1 days Difficulty
static const int64 nTargetSpacing = 5 * 60; // SuperGirlCoin: 5 minutes Block
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SuperGirlCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SuperGirlCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3dd793dd584f9f4d69302fa23363514e1f877a977590ca26b5c913a7335e811b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "HindutanTimes 03/04/2019, India’s first professional Kho-Kho league launched";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 *COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0441efb59ae0bf5e569ef511c76955eba15555f48214c7cb591add53883dc5573f577342169a73a182e052ce9d6f3a37cae5abd186d0e8c9ea2632a45412b71fc7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1554293923;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085241825;

        if (fTestNet)
        {
            block.nTime    = 1554293912;
            block.nNonce   = 386935125;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x0490c9012c6f9aef2e40126cbaeeb4a8633bf4c341fa7c56837bd6f1e0a11588ex"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdf }; // SuperGirlCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SuperGirlCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SuperGirlCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SuperGirlCoinMiner(CWallet *pwallet)
{
    printf("SuperGirlCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("supergirlcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SuperGirlCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SuperGirlCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SuperGirlCoinMiner, pwallet));
}
